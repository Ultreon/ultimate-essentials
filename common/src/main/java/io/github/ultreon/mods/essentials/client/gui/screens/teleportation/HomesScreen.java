...
import io.github.ultreon.mods.essentials.network.homes.DeleteHomePacket;
import io.github.ultreon.mods.essentials.network.homes.TeleportHomePacket;
...
public class HomesScreen extends InternalListScreen {
    private static final List<Home> receivedHomes = new ArrayList<>();
    ...
    public HomesScreen(List<Home> homes) {
    ...
    public static void handleHome(Home home) {
        HomesScreen.receivedHomes.add(home);
    }

    public static void handleListEnd() {
        Minecraft.getInstance().setScreen(new HomesScreen(receivedHomes));
    }
    ...
    public static class ListWidget extends ContainerObjectSelectionList<ListWidget.Entry> {
        ...
        private final List<Home> homes;
        private final HomesScreen screen;

        public ListWidget(HomesScreen screen, List<Home> homes, Minecraft minecraft, int width, int height, int top, int bottom, int itemHeight) {
            super(minecraft, width, height, top, bottom, itemHeight);
            this.mc = minecraft;

            for (Home home : homes) {
                this.addEntry(new Entry(this.mc, screen, home.getName(), home.getPosition().toString().replace("(", "").replace(")", "").replaceAll("(\\.\\d\\d)\\d*", "$1"), home));
            }
        ...
        public void search(String text) {
            ...
            for (Home home : this.homes) {
                if (StringUtils.contains(home.getName(), text)) {
                    this.addEntry(new Entry(this.mc, screen, home.getName(), home.getPosition().toString().replace("(", "").replace(")", "").replaceAll("(\\.\\d\\d)\\d*", "$1"), home));
                }
            }
        }
        ...
        public static class Entry extends ContainerObjectSelectionList.Entry<Entry> {
            ...
            public Entry(Minecraft minecraft, HomesScreen screen, String title, String description, Home home) {
                this.mc = minecraft;
                ...
                this.teleportBtn = new ImageButton(4, 28, 16, 16, 64, 0, 16, LIST_ICONS, 256, 256, (p_244751_4_) -> {
                    ...
                    if (teleportBtn.visible) {
                        Networking.get().sendToServer(new TeleportHomePacket(home));
                    }
                ...
                this.deleteBtn = new ImageButton(20, 28, 16, 16, 48, 0, 16, LIST_ICONS, 256, 256, (p_244751_4_) -> {
                    ...
                    if (deleteBtn.visible) {
                        Networking.get().sendToServer(new DeleteHomePacket(home.asReference()));
                    }
            ...
            public void render(@NotNull GuiGraphics gfx, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
                ...
                Objects.requireNonNull(deleteBtn).visible = true;
                ...
                // Teleport button render.
                this.teleportBtn.setX(left + (width - teleportBtn.getWidth() - deleteBtn.getWidth() - 4));
                ...
                this.deleteBtn.setX(left + (width - deleteBtn.getWidth() - 4));
